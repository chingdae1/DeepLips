...
from dataLoader import to_int, videoDataset
...
def train(watch_input_tensor, target_tensor,
    ...
    #sos token
    spell_input = torch.tensor([[37]]).repeat(watch_outputs.size(0), 1).to(device)
    ...
    if train:
        ...
        for di in range(target_length):
            ...
            #     print('output : ', to_int[int(topi.squeeze(1)[0])], 'label : ', to_int[int(target_tensor[0, di])])
...
def trainIters(n_iters, videomax, txtmax, data_path, batch_size, worker, ratio_of_validation=0.0001, learning_rate_decay=2000, save_every=30, learning_rate=0.01):
    ...
    watch = nn.parallel.DistributedDataParallel(watch).to(device)
    spell = nn.parallel.DistributedDataParallel(spell).to(device)
